...
namespace aengine::detail
{
    ...
    Expected<PlaybackHandle> AudioEngineBase::Play(const PlaybackDescription& description)
    {
        ...
        for (auto index = std::size_t { 0U }; index < this->PlaybackSlots.size(); ++index)
        {
            ...
            if (slot.Playback == nullptr && !slot.IsRetired)
            {
                ...
                return this->MakePlaybackHandle(static_cast<std::uint32_t>(index), slot.Version);
            }
        }
        ...
        return this->MakePlaybackHandle(index, this->PlaybackSlots.back().Version);
    }
    ...
    {
        slot.Playback.reset();
        if (slot.Version == std::numeric_limits<std::uint64_t>::max())
        {
            slot.IsRetired = true;
            return;
        }
