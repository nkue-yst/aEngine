...
namespace aengine
{
    ...
    public:
        ...
        {
            return this->GenerationValue != 0U && this->OwnerTokenValue != 0U;
        }
        ...
        [[nodiscard]] constexpr std::uint64_t Generation() const
        {
    ...
    private:
        ...
        constexpr Handle(std::uint32_t index, std::uint64_t generation, std::uint64_t ownerToken)
            : IndexValue(index)
    ...
    protected:
        IAudioEngine();
        [[nodiscard]] PlaybackHandle MakePlaybackHandle(std::uint32_t index, std::uint64_t generation) const;
